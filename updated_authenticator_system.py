...
# Slide Animation Switch Functions
def switch_to_login(root):
    global view_frame
    new_view = ctk.CTkFrame(right_frame, fg_color="#151515")
    create_login_view(new_view, root)
    new_view.place(x=right_frame.winfo_width(), y=0, relwidth=1, relheight=1)

    steps = 25
    step_size = right_frame.winfo_width() / steps

    def animate(step=0):
        global view_frame
        if step < steps:
            new_x = right_frame.winfo_width() - step_size * (step + 1)
            old_x = -step_size * (step + 1)
            new_view.place(x=new_x)
            if view_frame is not None:
                view_frame.place(x=old_x)
            root.after(12, lambda: animate(step + 1))
        else:
            if view_frame is not None:
                view_frame.destroy()
            view_frame = new_view
...
def switch_to_register(root):
    global view_frame
    new_view = ctk.CTkFrame(right_frame, fg_color="#151515")
    create_register_view(new_view, root)
    new_view.place(x=right_frame.winfo_width(), y=0, relwidth=1, relheight=1)
